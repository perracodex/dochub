...
#### Process
1. **Client Request**:
   - The client requests a signed URL by providing the document's UUID (`document_id`).
...
1. **Client Request**:
    - The client uses the signed URL to request the document download.
...
3. **Token and Signature Verification**:
    ...
    - If verification succeeds:
        - The server retrieves the document ID from the decrypted token.

4. **Document Retrieval**:
    - The server fetches the document file reference using the document ID.
    - If the document is not found, the server responds with a `404 Not Found` status.

5. **File Streaming**:
    - If the document is found, the server prepares to stream the document file to the client.
